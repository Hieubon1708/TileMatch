...
public class GameLevelManager : MonoBehaviour
{
    ...
    void Start()
    {
        slotParentTranform.localPosition = Vector2.zero;
        slotBG.gameObject.SetActive(false);
        originalSlotBgPos = slotBG.transform.position;
        SpriteRenderer sp = slotBG.GetComponent<SpriteRenderer>();
        sp.size = new Vector2(8.675f, 1.5f);
        if (Config.CheckTutorial_1())
        {
            sp.sortingLayerName = "TutUI";
        }
        InitListMoveType();
