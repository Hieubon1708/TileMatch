...
public class SpinPopup : MonoBehaviour
{
    ...
    public void TouchWatchAds()
    {
        ...
        if (ACEPlay.Bridge.BridgeController.instance.IsRewardReady())
        {
            ...
            ACEPlay.Bridge.BridgeController.instance.ShowRewarded("spin_tilematch_reward", onReward);
        }
        else
        {
            lockGroup.gameObject.SetActive(false);
    ...
    {
        Debug.LogError("Time : " + Time.timeScale);
        Debug.LogError("Time 2 : " + Time.deltaTime);
        Debug.LogError("Time 3 : " + Time.fixedDeltaTime);
        Debug.LogError("Time 4 : " + DOTween.timeScale);
